...
def _create_and_backfill(
    conn, name, model="test-model", dim=DIM, strategy="mechanical"
):
...
def test_compare_spaces_identical(mock_es, tmp_path):
    ...
    result = compare_spaces(
        conn, "test query", "space_a", "space_b", top_k=5, mode="vec"
    )
...
def test_compare_spaces_empty_results(mock_es, tmp_path):
    ...
    result = compare_spaces(
        conn, "test query", "empty_a", "empty_b", top_k=10, mode="vec"
    )
...
def test_compare_spaces_one_empty(mock_es, tmp_path):
    ...
    result = compare_spaces(
        conn, "test query", "full_space", "empty_space", top_k=10, mode="vec"
    )
...
def test_compare_spaces_cross_strategy_warning(mock_es, tmp_path):
    ...
    result = compare_spaces(
        conn, "test query", "mech_space", "sem_space", top_k=10, mode="vec"
    )
...
def test_batch_compare_spaces(mock_es, tmp_path):
    ...
    result = batch_compare_spaces(
        conn, "batch_a", "batch_b", queries, top_k=5, mode="vec"
    )

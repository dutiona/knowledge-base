...
from knowledge_base.server import _validate_bib_path
...
def test_export_bibtex_tool_uses_validated_path(tmp_path, monkeypatch):
    ...
    with patch.object(server, "_validate_bib_path", return_value=redirected) as mock:
        raw_input = str(tmp_path / "original.bib")
        result = json.loads(server.export_bibtex_tool(output_path=raw_input))
...
def test_sync_bibtex_tool_uses_validated_path(tmp_path, monkeypatch):
    ...
    with patch.object(server, "_validate_bib_path", return_value=redirected) as mock:
        ...
        with patch.object(
            server, "sync_bibtex", return_value={"synced": 1, "skipped": 0}
        ) as sync_mock:
            server.sync_bibtex_tool(output_path=raw_input)
            mock.assert_called_once_with(raw_input)

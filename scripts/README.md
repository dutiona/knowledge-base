...
```bash
scripts/sync-labels.sh    --dry-run   # then without --dry-run to apply
scripts/setup-projects.sh --dry-run   # then apply → writes scripts/.pm-ids.env
# (USER: build views, add "In Review" to Status, set KB_PROJECT_TOKEN — see below)
scripts/gen-mapping.sh                 # read-only → scripts/mapping.tsv
#   ⛔ review/approve mapping.tsv (resolve every ⚠ row) before the next step
scripts/migrate-issues.sh --dry-run    # then apply
```

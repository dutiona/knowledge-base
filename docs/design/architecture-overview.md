...
**`embed_swap.py`** -- Embedding space lifecycle management. Manages multiple embedding spaces (each with its own sqlite-vec virtual table) via a registry (`embed_spaces`). Supports create, backfill, promote, deprecate, and cleanup operations for zero-downtime model migration, A/B comparison, and rollback. The `re_embed()` convenience wrapper orchestrates the full lifecycle (create -> backfill -> promote) and re-embeds folder summaries. Promotion atomically swaps the active space and syncs the config table.
...
**`jobs.py`** -- Background job queue for long-running tasks. A singleton daemon thread (`_JobWorker`) processes jobs sequentially from the `jobs` table. Supports `extract_structure`, `extract_figures`, and `auto_relate` job types. Includes crash recovery (resets stale `running` jobs to `pending` on startup) and deduplication (reuses existing pending/running jobs for the same paper+type+params). `auto_relate` is the first auto-triggered job -- scheduled when a paper is registered with a `source_uri`. Stale `"similar"` relationships are cleaned up on reingest (per-paper) and re-embed (corpus-wide).
